...
export function CalendarView({
  ...
  const allEvents = useMemo(() => buildEventMap(weekDates), [weekDates, filter, campusFilter]);
  const monthEvents = useMemo(() => buildEventMap(monthGrid), [monthGrid, filter, campusFilter]);
  ...
  useEffect(() => {
    if (viewMode === "week" && weekScrollRef.current) {
      const allWeekEvents = Object.values(allEvents).flat();
      if (allWeekEvents.length > 0) {
        const firstStart = Math.min(...allWeekEvents.map((e) => e.startHour));
        const scrollTo = Math.max((firstStart - HOUR_START - 0.5) * HOUR_HEIGHT, 0);
        weekScrollRef.current.scrollTop = scrollTo;
      }
    }
  }, [weekOffset, allEvents, viewMode]);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="bg-white border-b border-gray-100 px-4 pt-2 pb-1">
        {/* 表示モード切替 */}
        <div className="flex gap-0.5 bg-gray-100/70 rounded-xl p-0.5 mb-2">
          {([
            { label: "月", value: "month" as ViewMode },
            { label: "週", value: "week" as ViewMode },
              ...
              {weekDates.map((date, i) => {
                ...
                return (
                  <div key={i} className="flex-1 text-center py-1.5">
                    ...
                    {count > 0 && (
                      <div className="text-[8px] font-bold text-chuo/50">{count}件</div>
                    )}
                  </div>
                );
              })}
          ...
          <div className="relative" style={{ height: totalHeight }}>
            ...
            {Array.from({ length: HOUR_END - HOUR_START }, (_, i) => (
              <div key={i} className="absolute w-full flex items-start" style={{ top: i * HOUR_HEIGHT }}>
                <div className="w-10 shrink-0 text-right pr-1 -mt-2">
                  <span className="text-[10px] text-gray-300 font-medium">{HOUR_START + i}</span>
                </div>
            ...
            <div className="absolute top-0 bottom-0 left-10 right-0 flex">
              {weekDates.map((date, dayIdx) => {
                const iso = toISO(date);
                const dayEvents = allEvents[iso] || [];
                const dayLayouted = layoutEvents(dayEvents);

                return (
                  <div key={dayIdx} className="flex-1 relative border-l border-gray-50 first:border-l-0">
                    {dayLayouted.map(({ circle, eventIdx, startHour, endHour, col, totalCols }) => {
                      ...
                      const height = Math.max(duration * HOUR_HEIGHT - 2, 24);
                      ...
                      return (
                        <button
                          ...
                          className={`absolute rounded-lg px-1 py-0.5 text-left transition-all active:scale-[0.97] overflow-hidden ${
                            isSports
                        ...
                        >
                          <div className="flex items-center gap-0.5">
                            ...
                            <span className="text-[9px] font-bold text-gray-700 truncate leading-tight">
                              {circle.name}
                            </span>
                          </div>
                          {height > 32 && (
...
function WeekCurrentTimeLine() {
  ...
  return (
    <div className="absolute left-10 right-0 flex items-center z-20 pointer-events-none" style={{ top }}>
      <div className="w-2 h-2 rounded-full bg-chuo -ml-1" />
      <div className="flex-1 h-[1.5px] bg-chuo/60" />
    </div>
  );
